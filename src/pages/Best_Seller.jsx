...
/* ---------------- strict “oils” category filter ---------------- */
const norm = (s = "") => s.trim().toLowerCase();

// true if any category/slug equals "oils"
const categoryMatchesOils = (product) => {
  ...
  return (
    norm(catName) === "oils" ||
    norm(slug) === "oils" ||
    arrNames.includes("oils") ||
    arrSlugs.includes("oils")
  );
};

// Only use keyword fallback if category data missing
const OIL_KEYWORDS = [
  "oil",
  "oils",
  "groundnut",
...
const looksLikeOilFallback = (p) => {
  const hay = `${p?.name || ""} ${p?.description || ""}`.toLowerCase();
  return OIL_KEYWORDS.some((kw) => hay.includes(kw));
};
...
export default function Best_Seller() {
  ...
  // Fetch ONLY oil-category products (server filter if possible → otherwise client filter)
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);

        // Try common server-side filters first (fastest & cleanest)
        const attempts = [
          () => axios.get(`${API_BASE}/api/products`, { params: { category: "oils" } }),
          () => axios.get(`${API_BASE}/api/products`, { params: { categorySlug: "oils" } }),
          () => axios.get(`${API_BASE}/api/products`, { params: { categoryName: "Oils" } }),
          () => axios.get(`${API_BASE}/api/products?category=oils`),
        ];
        ...
        // If none succeeded/returned data, fetch all and filter strictly on client
        if (!serverList) {
          const all = await axios.get(`${API_BASE}/api/products`);
          serverList = Array.isArray(all.data) ? all.data : all.data?.products || [];
        }

        const onlyOils = serverList.filter(isOilProductStrict);

        const formatted = onlyOils.map((product) => {
          const { sale, mrp } = computeVariantPricePair(product);
          const image = buildImageUrl(product.images?.[0] || "/media/default.jpg");
          return {
            id: product._id,
            ...
            image,
            originalPrice: mrp, // MRP (strike)
            discountedPrice: sale, // selling price
          };
        });
  ...
  const handleCardClick = (name) => {
    // PDP route expects no spaces based on your previous screens
    navigate(`/oil-products/${name.replace(/\s+/g, "")}`);
  };
                ...
                >
                  {filtered.length === 0 ? (
                    <p style={{ textAlign: "center", gridColumn: "1 / -1" }}>
                      {products.length ? "No products match your search." : "No oil products found."}
                    </p>
                  ) : (
                    filtered.map((item) => {
                      const hasDiscount =
